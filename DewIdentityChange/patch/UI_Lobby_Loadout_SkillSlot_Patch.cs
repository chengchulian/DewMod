using System.Linq;
using System.Reflection;
using HarmonyLib;
using UnityEngine;

namespace DewIdentityChange.patch;

[HarmonyPatch(typeof(UI_Lobby_Loadout_SkillSlot))]
public static class UI_Lobby_Loadout_SkillSlot_Patch
{
    private static readonly FieldInfo SkillsField =
        AccessTools.Field(typeof(UI_Lobby_Loadout_SkillSlot), "_skills");

    private static readonly MethodInfo SetSkill =
        AccessTools.PropertySetter(typeof(UI_Lobby_Loadout_SkillSlot), nameof(UI_Lobby_Loadout_SkillSlot.skill));

    [HarmonyPrefix]
    [HarmonyPatch("OnLoadoutChanged")]
    public static bool OnLoadoutChanged_Prefix(UI_Lobby_Loadout_SkillSlot __instance)
    {
        var loadout = __instance.loadout ?? DewPlayer.local?.selectedLoadout;
        var hero = GetCurrentHero(__instance);
        if (loadout == null || hero == null)
        {
            return false;
        }

        var skills = hero.GetComponent<HeroSkill>()
            .GetLoadoutSkills(__instance.type)
            .Where(skill => skill != null)
            .ToArray()
            .ToAssetRefs();
        SkillsField.SetValue(__instance, skills);

        if (skills == null || skills.Length == 0)
        {
            SetSkill.Invoke(__instance, new object[] { default(AssetRef<SkillTrigger>) });
            __instance.skillIcon.enabled = false;
            __instance.selectionCountText.text = "0/0";
            __instance.selectionGroupObject.SetActive(false);
            __instance.newObject.SetActive(false);
            return false;
        }

        int index = Mathf.Clamp(loadout.GetSkill(__instance.type), 0, skills.Length - 1);
        AssetRef<SkillTrigger> skill = skills[index];
        SetSkill.Invoke(__instance, new object[] { skill });

        __instance.skillIcon.enabled = true;
        __instance.skillIcon.sprite = skill.asset.configs[0].triggerIcon;
        __instance.selectionCountText.text = $"{index + 1}/{skills.Length}";
        __instance.selectionGroupObject.SetActive(skills.Length > 1);
        UpdateHasNewStatus(__instance, hero, skill);
        return false;
    }

    private static Hero GetCurrentHero(UI_Lobby_Loadout_SkillSlot slot)
    {
        string heroType = DewPlayer.local?.selectedHeroType;
        if (!string.IsNullOrEmpty(heroType))
        {
            var hero = DewResources.GetByShortTypeName<Hero>(heroType);
            if (hero != null)
            {
                return hero;
            }
        }

        return slot.hero.asset;
    }

    private static void UpdateHasNewStatus(
        UI_Lobby_Loadout_SkillSlot slot,
        Hero hero,
        AssetRef<SkillTrigger> selectedSkill)
    {
        if (slot.type == HeroSkillLocation.Movement || hero == null || selectedSkill.asset == null)
        {
            slot.newObject.SetActive(false);
            return;
        }

        bool hasNew = false;
        foreach (var skill in hero.GetComponent<HeroSkill>().GetLoadoutSkills(slot.type))
        {
            if (skill == null || selectedSkill.asset == skill)
            {
                continue;
            }

            if (DewSave.profileMain.skills.TryGetValue(skill.GetType().Name, out var data) &&
                data.isNewHeroOrHeroSkill)
            {
                hasNew = true;
                break;
            }
        }

        slot.newObject.SetActive(hasNew);
    }
}
